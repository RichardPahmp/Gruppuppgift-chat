...
/**
 * Extends JPanel. A JList customized for the <code>Message</code> class to
 * render both picture and text in the same list cell.
 * 
 * @author Erik Lundov
 *
 */
public class MessageList extends JPanel {
	 ...
	 * 
	 * @Param message A received <code>Message</code>
	 */
	...
	/**
	 * extends JPanel. Custom <code>ListCellRenderer</code> for the
		...
		public Component getListCellRendererComponent(JList<? extends Message> list, Message message, int index,
			...
			if (message instanceof TextMessage) {
				...
				if (mess.getReceivers().toString() != "[]") {
					messageReceivers = " till "
							+ mess.getReceivers().toString().substring(1, mess.getReceivers().toString().length() - 1);
				}
				label.setText("[" + mess.getTimeSent().format(DateTimeFormatter.ofPattern("HH:mm")) + "] "
						+ mess.getSender().getName() + ": " + mess.getText());
				...
				label.setText("[" + mess.getTimeSent().format(DateTimeFormatter.ofPattern("HH:mm")) + "] "
						+ mess.getNewUser().getName() + " has connected to the chat");
				return label;
				...
				label.setText("[" + mess.getTimeSent().format(DateTimeFormatter.ofPattern("HH:mm")) + "] "
						+ mess.getDisconnectedUser().getName() + " has disconnected from the chat");
				return label;
			}
